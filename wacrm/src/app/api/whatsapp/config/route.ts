...
export async function GET() {
  try {
    ...
    // `*` rather than a column list: it includes migration 043's
    // `provider` where present without breaking a database that hasn't
    // run it yet.
    ...
    // The account's one connection is MboWazap: there is no Meta token
    // to check, and treating the NULL token as corrupted would steer the
    // user to "Reset Configuration" — deleting the MboWazap row without
    // unlinking the bot session.
      ...
      return NextResponse.json(
        {
          ...
          reason: 'mbowazap',
          provider: 'mbowazap',
          message: MBOWAZAP_CONNECTED_MESSAGE,
        },
        { status: 200 }
      )
    }
...
export async function POST(request: Request) {
  try {
    ...
    // One connection per account. While MboWazap holds it, refuse before
    // touching Meta — disconnecting MboWazap (which also unlinks the bot
    // session) comes first.
    const { data: current } = await supabase
      ...
      .eq('account_id', accountId)
      .maybeSingle()
    if (whatsappProvider(current) === 'mbowazap') {
      return NextResponse.json(
        { error: MBOWAZAP_CONNECTED_MESSAGE, code: 'provider_conflict' },
        { status: 409 }
      )
    }
...
export async function DELETE() {
  try {
    ...
    // A MboWazap connection is removed from its own settings page, which
    // also unlinks the bot session; deleting the row here would leave
    // the paired number running with nothing tracking it.
    const { data: current } = await supabase
      .from('whatsapp_config')
      .select('*')
      .eq('account_id', accountId)
