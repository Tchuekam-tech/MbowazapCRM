...
async function processMessage(
  ...
  if (message.type === 'reaction') {
    await handleInboundReaction(supabaseAdmin(), {
  ...
  await flagBroadcastReplyIfAny(supabaseAdmin(), accountId, contactRecord.id)

  // Fan out to flows, automations, AI auto-reply, and public webhooks.
  await dispatchInboundLifecycle(supabaseAdmin(), {
    accountId,
    configOwnerUserId,
    contactId: contactRecord.id,
    conversationId: conversation.id,
