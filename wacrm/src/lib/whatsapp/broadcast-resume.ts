...
import { META_ONLY_MESSAGE, whatsappProvider } from '@/lib/whatsapp/provider';
...
export async function planBroadcastResume(
  ...
  // An account that switched to MboWazap can't resume a Meta broadcast:
  // it would message people who never wrote first.
  if (whatsappProvider(config) === 'mbowazap') {
    throw new BroadcastError('meta_only', META_ONLY_MESSAGE, 409);
  }
