...
//   2. loads the conversation + contact + the account's transport,
//   3. sends through it — Meta (with phone-variant retry + contact
//      auto-fix) or MboWazap (reply-only),
...
export async function sendMessageToConversation(
  ...
  // How the provider addresses this contact. On Meta that's the phone
  // number, or the business-scoped user ID for a customer who adopted a
  // WhatsApp username and whose number Meta withholds — reached through
  // Meta's `recipient` field instead of `to` (issue #519). On MboWazap
  // it's the phone number, or the WhatsApp LID. Phone stays preferred
  // when we have one: only it supports the trunk-prefix variant retry
  // below.
  const resolvedTarget = transport.resolveTarget(contact);
  if (!resolvedTarget) {
    throw new SendMessageError(
      'bad_request',
      contact?.phone
        ? 'Invalid phone number format'
        : transport.provider === 'mbowazap'
          ? 'Contact has no phone number or WhatsApp LID'
  ...
  // Resolve the reply target to its Meta/WhatsApp message_id. The parent must
  ...
  const attempt = async (target: string): Promise<string> => {
    ...
    if (messageType === 'interactive') {
      const result = await transport.sendInteractive({
        to: target,
  ...
  // Send — on Meta, retry across phone-number variants if Meta rejects
  // with "recipient not in allowed list"; persist a working variant
  // back to the contact so the next send goes straight through.
  let waMessageId = '';
  let workingPhone = sendTarget;
  try {
    // Variants only make sense for a phone number on Meta — a BSUID is
    // opaque and has exactly one correct form, and MboWazap sends to the
    // number WhatsApp itself reported, so both get a single attempt.
    const variants =
      hasValidPhone && transport.provider === 'meta'
    ...
    // MboWazap failures (reply-only refusal, bridge errors) arrive
    // already typed; everything else here is a Meta API error.
