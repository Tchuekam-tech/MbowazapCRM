...
import { loadTransport } from '@/lib/whatsapp/transport'
import {
  phoneVariants,
...
async function sendViaMeta(input: SendInput): Promise<{ whatsapp_message_id: string }> {
  ...
  // new tenancy column.
  // `*` rather than a column list so the Meta path keeps working on a
  // database that hasn't run migration 043 (contacts.wa_lid) yet.
  ...
  const transport = await loadTransport(db, input.accountId)

  // Phone number, or the business-scoped user ID when Meta has never
  // given us a number for this customer (issue #519) — or, on MboWazap,
  // the WhatsApp LID.
  ...
  // MboWazap has no templates and sends this text instead.
  ...
  // reliably land a message. MboWazap gets a single attempt.
  const variants =
    sendTarget.isPhone && transport.provider === 'meta'
  ...
  const content_text = input.kind === 'text' ? input.text : renderedTemplate
