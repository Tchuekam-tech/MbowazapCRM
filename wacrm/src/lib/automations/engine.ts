...
export interface AutomationContext {
  ...
  /** Whether outbound messages should be suppressed (e.g. Davila is active). */
...
async function shouldSkipOutboundMessage(
  db: ReturnType<typeof supabaseAdmin>,
  args: ExecuteArgs
): Promise<string | null> {
  if (args.context?.suppressReplies) {
    return 'skipped: replies suppressed by conversational brain'
  }
  if (args.contactId) {
    const { data: optOutTag } = await db
      .from('contact_tags')
      .select('tag_id, tags!inner(name)')
      .eq('contact_id', args.contactId)
      .eq('tags.name', 'Opted out')
      .limit(1)
      .maybeSingle()
    if (optOutTag) {
      return 'skipped: contact has opted out'
    }
  }
  return null
}
...
async function runStep(step: AutomationStep, args: ExecuteArgs): Promise<string> {
  ...
  switch (step.step_type) {
    case 'send_message': {
      const skip = await shouldSkipOutboundMessage(db, args)
      if (skip) return skip
