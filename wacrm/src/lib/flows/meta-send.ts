...
export async function engineSendText(
  ...
  const transport = await loadTransport(db, args.accountId)

  // Phone number, or the business-scoped user ID when Meta has never
  // given us a number for this customer (issue #519) — or, on
  // MboWazap, the WhatsApp LID.
  ...
  const attempt = async (target: string): Promise<string> => {
    const r = await transport.sendText({
      to: target,
      text: args.text,
      origin: args.aiGenerated ? 'ai' : 'flow',
      conversationId: args.conversationId,
    })
    return r.messageId
  }

  const variants = (sendTarget.isPhone && transport.provider === 'meta')
...
export async function engineSendMedia(
  ...
  const attempt = async (target: string): Promise<string> => {
    const r = await transport.sendMedia({
      to: target,
      kind: args.kind,
      link: args.link,
...
async function sendInteractiveViaMeta(
  ...
  const attempt = async (target: string): Promise<string> => {
    const r = await transport.sendInteractive({
      to: target,
